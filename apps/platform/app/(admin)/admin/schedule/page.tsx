...
export default async function SchedulePage() {
  ...
  try {
    // Fetch jobs for current month + next month (wider range for calendar view)
    const now = new Date();
    ...
    const scheduledJobs: CalendarJob[] = jobs
      ...
      .map((job) => {
        ...
        return {
          ...
          customerId: job.requestId, // Link to customer detail via service request
          cleaner: cleanerName,
        ...
        <div className="rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-6 text-white">
          <div>
            ...
            <p className="mt-1 text-sm text-brand-100">
              View and manage all scheduled jobs — replaces Booking Koala & Jobber
            </p>
          </div>
        </div>
