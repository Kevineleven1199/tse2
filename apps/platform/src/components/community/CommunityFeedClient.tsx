...
const InteractivePostCard = ({
  ...
  return (
    <div
      className={`rounded-2xl border border-brand-100 bg-white transition hover:shadow-md ${post.isPinned ? "ring-2 ring-brand-200" : ""}`}
    >
      <div className="p-5">
        {/* Header */}
        ...
        <div className="mt-3">
          <Link href={`/community/post/${post.id}`}>
            <h3 className="text-lg font-semibold text-accent transition hover:text-brand-600">
              {post.title}
            </h3>
          </Link>
...
export const CommunityFeedClient = ({
  ...
  const [allPosts, setAllPosts] = useState<CommunityPost[]>(seedPosts);
